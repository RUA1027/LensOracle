"""LensOracle 单阶段训练入口脚本。

该脚本负责把 `LensOracleTrainer` 驱动起来，并处理训练运行期的工程化职责：
1) 解析配置与运行参数；
...
_STAGE_RESTORATION = "restoration"
BEST_PERFORMANCE_FILENAME = "best_performance.json"


def _stage_tag(_stage: str) -> str:
    """把内部 stage 名映射为更短的文件名后缀。"""

    return _STAGE_RESTORATION
...
def _resolve_stage_metric_name(config, _stage: str) -> str:
    """读取用于 best 判定的配置指标名。"""
...
def _best_performance_template() -> Dict[str, Dict[str, Any]]:
    ...
    return {_STAGE_RESTORATION: {}, "latest": {}}
...
def _build_best_tracking_state(config, best_performance_state: Dict[str, Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
    ...
    return {stage: {"metric_name": metric_name, "maximize": maximize, "score": score}}


def _save_checkpoint_bundle(trainer, checkpoint_dirs: Tuple[Path, ...], filenames: Tuple[str, ...], epoch: int, global_step: int, val_metrics: Dict[str, Any]) -> None:
    """把同一 checkpoint 内容写入多个目录与多个文件名。"""

    for checkpoint_dir in checkpoint_dirs:
        checkpoint_dir.mkdir(parents=True, exist_ok=True)
        for filename in filenames:
            trainer.save_checkpoint(str(checkpoint_dir / filename), epoch=epoch, global_step=global_step, val_metrics=val_metrics)


def _resolve_auto_start_checkpoint(_requested_stage: str, fixed_checkpoints_dir: Path) -> Tuple[Optional[str], Optional[str], Optional[str]]:
    ...
    latest_restoration = fixed_checkpoints_dir / "latest_restoration.pt"
    best = fixed_checkpoints_dir / "best.pt"
    if latest_restoration.exists():
        return str(latest_restoration), None, "auto_resume_latest"
    if latest.exists():
...
def _stage_window(config, _requested_stage: str) -> Tuple[int, int]:
    """返回训练对应的 global_step 窗口 `[start, end)`。"""

    total = int(config.training.stage_schedule.stage3_iterations)
    return (0, total)


def _stage_display_name(_stage: str) -> str:
    """返回当前阶段的可读文本。"""
...
def _extract_validation_loss(metrics: Dict[str, Any], _stage: str) -> Optional[float]:
    """提取验证损失近似值。"""

    for key in ("val_loss", "MAE", "mae", "loss"):
...
def _save_periodic_checkpoint(trainer, checkpoints_dir: Path, epoch: int, global_step: int, val_metrics: Dict[str, Any], save_interval: int) -> bool:
    ...
    _save_checkpoint_bundle(
        ...
        filenames=("latest.pt", f"latest_{_stage_tag(_STAGE_RESTORATION)}.pt"),
        epoch=epoch,
...
def _maybe_save_best_checkpoint(trainer, checkpoints_dir: Path, epoch: int, global_step: int, val_metrics: Dict[str, Any], best_tracking_state: Dict[str, Dict[str, Any]]) -> bool:
...
def _train_one_cycle(trainer, loader, global_step: int, target_step: int) -> Tuple[Dict[str, float], int, Optional[Dict[str, Any]]]:
    ...
    counts: Dict[str, int] = {}
    last_batch = None
    progress = tqdm(loader, desc=f"restoration @step {global_step}", leave=False)
    ...
    if trainer.pending_accumulation_steps > 0:
        if global_step < target_step and trainer.flush_pending_gradients():
            global_step += 1
        else:
...
def _validate_one_epoch(trainer, loader, device: str, evaluator: PerformanceEvaluator) -> Dict[str, float]:
    """调用 evaluator 执行验证。"""
...
def _export_epoch_visuals(trainer, config, batch: Optional[Dict[str, Any]], epoch: int, output_dir: str, device: str) -> None:
    ...
    try:
        with torch.no_grad():
            ...
            if prior_table is not None and torch.is_tensor(prior_table):
                prior_table = prior_table[:1].to(device)
            else:
                prior_table = None
...
def main() -> None:
    ...
    parser.add_argument("--stage", type=str, default="restoration", choices=["restoration"])
    ...
    if resume_path is None and init_from_path is None:
        auto_resume, auto_init, reason = _resolve_auto_start_checkpoint("all", fixed_checkpoints_dir)
        if auto_resume:
    ...
    stage = _STAGE_RESTORATION
    loaders = {
        "train": build_mixlib_dataloader(config, mode="train", batch_size_override=int(schedule.stage3_batch_size)),
        "val": build_mixlib_dataloader(config, mode="val", batch_size_override=int(schedule.stage3_batch_size)),
    }
    ...
    while global_step < run_end_step:
        ...
        # 8.1 训练一个小周期。
        train_metrics, global_step, last_batch = _train_one_cycle(trainer, loaders["train"], global_step, cycle_target)
        ...
        if should_validate:
            val_metrics = _validate_one_epoch(trainer, loaders["val"], device, evaluator)
            trainer.log_to_tensorboard(val_metrics, global_step, prefix="val")
            last_val_metrics = val_metrics
            _print_metric_table("Validation metrics:", val_metrics)
            trainer.update_best_metrics(val_metrics)
            if _maybe_save_best_checkpoint(trainer, checkpoints_dir, virtual_epoch, global_step, val_metrics, best_tracking):
                ...
                _save_checkpoint_bundle(trainer, tuple(p for p in checkpoint_save_dirs if str(p) != str(checkpoints_dir)), ("best.pt", f"best_{_stage_tag(stage)}.pt"), virtual_epoch, global_step, val_metrics)
        # 8.3 周期性保存 latest checkpoint。
        if _save_periodic_checkpoint(trainer, checkpoints_dir, virtual_epoch, global_step, last_val_metrics, save_interval):
            _save_checkpoint_bundle(trainer, tuple(p for p in checkpoint_save_dirs if str(p) != str(checkpoints_dir)), ("latest.pt", f"latest_{_stage_tag(stage)}.pt"), virtual_epoch, global_step, last_val_metrics)
        best_state["latest"] = {
            "stage": stage,
            "epoch": int(virtual_epoch),
            "global_step": int(global_step),
        ...
        if _should_export_visuals(config, virtual_epoch):
            _export_epoch_visuals(trainer, config, last_batch, virtual_epoch, output_dir, device)
        virtual_epoch += 1

    # 9) 训练结束后保存 final/latest，并关闭 tensorboard writer。
    _save_checkpoint_bundle(trainer, checkpoint_save_dirs, ("final_model.pt", "latest.pt", f"latest_{_stage_tag(stage)}.pt"), max(0, virtual_epoch - 1), global_step, last_val_metrics)
